...
print("local:", paras)
wandb.init(project="foil", config=paras, name=args.name, mode="disabled")
...
# env = gym.vector.make('foil-v0', num_envs=1, config=paras,local_port=8686)
...
agent = TPPO(paras)
agent.load('F:/foilRL/trained_model/actor_1080.pth') #'/mnt/nasdata/runji/LilyPad/models/foil/rsac/run197/trained_model/actor_offline.pth')
# imitation learning
...
for i in tqdm(range(1)):
    ...
    while not done and epsoide_length < 8000:
        action = agent.choose_action(obs, train=False)
        print("step", epsoide_length, " time", sum(dt_ls)," action", action[0][0], action[0][1])
        # print("==================action:", action, "==================================")
        # print("===============================", type(obs), obs, "==========================================")
        coff = 0 if epsoide_length <= 0 else 1
        next_obs, reward, done, info = env.step(action[0]*coff)
    ...
    print("action:", action, "; state: ", next_obs, "; reward:", reward, "ct:", ct_avg, 'eta:', eta_avg)
    Gt = 0
    ct_sum, eta_sum, cp_sum, fx_sum = 0, 0, 0, 0
    ct_ls, cp_ls, fx_ls, dt_ls = [], [], [], []
    epsoide_length = 0
