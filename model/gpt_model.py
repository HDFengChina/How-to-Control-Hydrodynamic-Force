...
class GPT(nn.Module):
    ...
    def __init__(self, config, args, model_type='actor'):
        super().__init__()
        ...
        if self.model_type == 'actor':
            ...
            if args.moe:
                self.fake_head = nn.Linear(config.n_embd, config.n_embd)
                self.fake_head = deepspeed.moe.layer.MoE(hidden_size=config.n_embd,
                                                    expert=self.fake_head,
        ...
        if self.model_type == 'actor':
            logger.info("number of actor parameters: %e", sum(p.numel() for p in self.parameters()))
        elif self.model_type == 'critic':
            logger.info("number of critic parameters: %e", sum(p.numel() for p in self.parameters()))
    ...
    def forward(self, states, pre_actions=None, rtgs=None, timesteps=None):
        ...
        if self.mode == 'rtgs_state_action':
        ...
        x = self.blocks(x)
        x = self.ln_f(x)
        if self.model_type == 'actor' and self.args.moe:
            x, _, _ = self.fake_head(x)
            logits = self.head(x)
        else:
            logits = self.head(x)
        # logits = self.head(x)
