...
class BDIM{
  ...
  void update(){
    ...
    c.eq(del.times(rhoi.times(dt))); //times is multiply, eq is equal
    u0.eq(u);
...
*/
    ...
    if(mu1) u.plusEq(del1.times(du.normalGrad(wnx,wny))); //velocity is here in fluid field
    u.setBC();
    p = u.project(coeff,p);    //pressure is here
  }
  ...
  void write( String name ){
    ...
    for ( int i=0; i<n; i++ ){
    for ( int j=0; j<m; j++ ){
      output.println(""+u.x.a[i][j]+", "+u.y.a[i][j]+", "+p.a[i][j]); //here can output velocity and pressure
    }}
    output.flush(); 
    output.close();
  }
