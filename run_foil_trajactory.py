...
def prepare_arguments():
    ...
    args.name = f"[Debug]foil_newEnvV7_newSAC_newObs_{args.seed}"
...
agent = RSAC(paras)
#agent.load('/mnt/nasdata/runji/LilyPad/models/foil/rsac/run208/trained_model/actor_offline.pth')
# imitation learning
...
buffer_new_data = 0
agent.reset_optim() # change the mode from offline to online
for i in tqdm(range(config['epochs'])):
    # rollout in env  -  rollout()
    agent.reset_rnn()
    ct_sum, eta_sum, cp_sum, fx_sum = 0, 0, 0, 0
    ct_ls, cp_ls, fx_ls, dt_ls = [], [], [], []
    while not done and epsoide_length < 800:
        action = agent.choose_action(obs)
        #print("==================action:", action, "==================================")
        #print("===============================", type(obs), obs, "==========================================")
        next_obs, reward, done, info = env.step(action["action"][0][0])
        #print("==================reward:", reward, "==================================")
        ...
        agent.add_experience(
            {"states": np.array(obs).astype(float).reshape(1, observation_dim), "states_next": np.array(next_obs).astype(float).reshape(1, observation_dim), "rewards": reward, "dones": np.float32(done), "id": epsoide_length})
        obs = next_obs
        Gt += reward
        ct_sum += info['ct']
        eta_sum += info['eta']
    ...
    wandb.log({"Gt": Gt, "length": epsoide_length, "episode_num": epsoide_num, "avg_reward": avg_reward, "effective_avg": eff_avg_reward, "ct": ct_avg, 'eta': eta_avg, "policy_entropy": agent.ent, "alpha": agent.alpha, "time": np.sum(dt_ls)}) # avg should remove the final step reward, since it can be too large
